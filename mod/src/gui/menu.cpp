...
namespace ml::gui
{
    ...
    static void TabGeneral(Config& c)
    {
        ...
        Help(TR("Searching a body or a carcass hands over everything it holds, and the mod only sees it once it is in the bag. On: whatever of that your item rules, tags, classes or value floor refuse is put straight back on the ground beside you, through the same drop the game uses when you drop something yourself, one pile per stack. It covers the mod's own searches only. A body you search yourself keeps what it paid, and so does one a pet loots. Money, documents, quest items and protected items stay in the bag."));
