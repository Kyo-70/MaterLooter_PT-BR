...
Rule order for an identified item: item override, tag never, protected tags (memory fragments, mechanism parts), tag always, dev/quest/unsellable filters, copper value floor, class rule, then loot. Built-in protections apply before any of that: quest and shop objects, locked nodes, your own equipment and bag contents, anything you dropped yourself, gear worn by others, mechanism parts, container stacks, memory triggers, and anything the game's own Take-or-Steal check calls theft (unless you opt in).

Pets and companions are outside all of that: the game lets one loot any dropped item and any lootable body, and nothing in its rules looks at the item. Two switches under General stop a pet taking loose items or looting bodies at all, by answering no to the game's own question for each. The pet filter under General, off by default, tells a pet no before it reaches for a loose item your rules refuse and judges what lands from a body. A pet's body search reaches the event queue under the pet's own id, so the mod compares the inventory before and after it and deletes any increase that your item override, tag or class rule refuses, largest stack first, through the game's own TrocTrDeleteItemFromInventoryOnceTimer. Skip unsellable items and the value floor never delete anything; they only stop the mod taking it. That event names the slot and the slot's own instance, so it cannot touch anything but the stack it was aimed at. Quest, protected and dev items are never deleted, and a notice on screen lists what was. A refused item you pick up by hand is always kept. The pet was told no before it could take one, and while a mercenary is out the mod keeps any row you picked up yourself out of the bag diff it deletes from.

A body or a carcass the mod searches is outside the rules too, since the game pays out everything it holds before anything can be judged. "Drop refused loot from bodies" under General, off by default, puts back on the ground what the rules refuse. The routine is the one TrocTrDiscardItemReq's parser calls, found as the first call after GetInventoryHolder in that parser, and it works only on the game's server thread, which carries a per-thread context the others lack. So the mod detours the server's parse of the search, reads the carried bag slot by slot before and after, and drops each instance that is new or the amount a stack grew by, at the transform's world position of the body being played. That covers the mod's own searches and a search the event queue saw a pet or a companion raise; for those the bag is the one GetInventoryHolder gives for the pet, and the drop still lands beside the player. Searches made by hand, money, documents, quest, protected and dev items are left alone. [DROP.md](../docs/investigations/DROP.md) has the layout and the evidence.

What you drop out of the bag yourself is watched from the same parser, which the mod detours whatever that switch says. Before the parse runs it reads the slot the payload names, so it knows the item row, and tells the scan to expect that item at the world position the payload asks for. The scan turns that position into its own frame with the offset between the played body's two readings of itself, and refuses objects of that row within 5 m of it that it first saw after the drop, nearest first and no more than the amount dropped, as "you dropped this" for the rest of the session. The world object keeps the bag slot's instance id, so that id and the item row are refused as well from the moment the scan hears of the drop, which covers an object that lands further off or turns up late. A drop made while the scan is off is matched when you are next near the spot. The log says what it matched, and says so when a drop turned up nothing.

Swapping characters is noticed by who walks. As Kliff the actor the game raises your events under is your body and walks with you; as Damiane or Oongka it sits wherever the game parked it, jumps when the game moves it, and never walks. A few steps after a swap put the scan on the right character: the actor walking means Kliff, a played body walking means her, and between Damiane and Oongka the body that walked in the last three seconds takes the centre from one that has stood for ten.
...
1. Take [docs/MasterLooter.template.txt](../docs/MasterLooter.template.txt), which holds all 450 strings. It is generated from the source by `scripts/make_translation_template.py`, so it covers the whole menu.
...
- Creatures are named from the game's own data, not from their model name. A creature actor carries its row in the character table at `status+0x30`, which the mod resolves to a string key and looks up in the creature table, so it knows a Tench from a Ricefish before deciding. That means a per-item override refuses a catch: set `Item_Tench` to never on the Items tab and tenches are left alone while other fish are still taken. A creature the game gives no item of its own is judged by the class its row carries instead, so refusing the class insect leaves Firefly Colonies alone; tag and item rules cannot reach it. Model-name matching remains as the fallback for the few this cannot name, and those are caught by category only.
- Gather nodes are identified by their prefab, matched against a table of 2,022 node kinds built from the game's own data. A node outside that table is identified by what lands in your bag when it is gathered, and is otherwise left alone until you turn Unidentified nodes on.
