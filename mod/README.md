...
Pets and companions are outside all of that: the game lets one loot any dropped item and any lootable body, and nothing in its rules looks at the item. Two switches under General stop a pet taking loose items or looting bodies at all, by answering no to the game's own question for each. The pet filter under General, off by default, tells a pet no before it reaches for a loose item your rules refuse and judges what lands from a body. A pet's body search reaches the event queue under the pet's own id, and its pick-up of a loose item reaches it under yours, exactly like one you did by hand, so the mod compares the inventory before and after either and deletes any increase that your item override, tag, class, value floor or no-sell rule would have refused, largest stack first, through the game's own TrocTrDeleteItemFromInventoryOnceTimer. That event names the slot and the slot's own instance, so it cannot touch anything but the stack it was aimed at. Quest, protected and dev items are never deleted, and a notice on screen lists what was. A refused item you pick up by hand is kept, because the pet was told no before it could take one. A hired mercenary is the exception. Nobody has checked that it asks the same question, so while one is out a refused item that lands is deleted whoever picked it up.
...
1. Take [docs/MasterLooter.template.txt](../docs/MasterLooter.template.txt), which holds all 416 strings. It is generated from the source by `scripts/make_translation_template.py`, so it covers the whole menu.
