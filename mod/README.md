...
A body or a carcass the mod searches is outside the rules too, since the game pays out everything it holds before anything can be judged. "Drop refused loot from bodies" under General, off by default, puts back on the ground what the rules refuse. The routine is the one TrocTrDiscardItemReq's parser calls, found as the first call after GetInventoryHolder in that parser, and it works only on the game's server thread, which carries a per-thread context the others lack. So the mod detours the server's parse of its own search, reads the carried bag slot by slot before and after, and drops each instance that is new or the amount a stack grew by, at the transform's world position of the body being played. Searches made by hand, money, documents, quest, protected and dev items are left alone. [DROP.md](../docs/investigations/DROP.md) has the layout and the evidence.
